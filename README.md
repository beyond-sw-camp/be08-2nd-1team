...
|**김도하**|**김민지**|**이은서**|**전현규**|**조은희**|
|:--:|:--:|:--:|:--:|:--:|
|꽤나 준비를 하는데 넉넉한 시간이라고 생각했는데 spring 강의가 시작되기도 하였고, 개발 과정에서 개선 사항이 계속 보이다 보니 시간이 부족해졌습니다. 하지만 팀원들이 모두 열심히 할 일을 하여 결과물을 낼 수 있었다고 생각합니다. |개인 사정으로 중간에 하차하게 되어 미안한 마음이 있었지만 끝까지 최선을 다해 마무리해준 팀원들에게 감사의 인사를 드리고 싶습니다. 강의에서 배운 swagger를 처음 사용해 보면서 swagger의 편리함을 알게 된 프로젝트였고 프로젝트를 진행하며 배웠던 부분을 복습할 수 있는 기회가 된 것 같아 좋았습니다. 다들 정말 수고 많으셨습니다! |처음해보는 백엔드 프로젝트였지만, 멋있는 팀원들과 함께해서 포기하지 않고 끝까지 할 수 있었던것 같습니다. 넉넉하게 준비했다고 생각했지만 생각했던 것보다 많이 어려워서 시간이 부족하였습니다. 하지만, 팀원들의 도움으로 멋있는 결과물이 나온것 같다고 생각합니다! 모두 수고 많으셨습니다!|여러 팀원과 git을 사용해서 코드를 공유하며 같이 프로젝트를 구현해 볼 수 있는 좋은 기회였던 것 같습니다. spring과 spring boot로 API를 만들고, db도 연결해 보고, swagger를 사용해 테스트까지 해보며 좋은 경험을 쌓을 수 있던 시간이었습니다. 익숙하지 않은 코드가 많아 시간이 오래 걸렸지만, 팀원들 모두 고생한 결과 잘 마무리된 것 같습니다.|수업 진도에 맞춰서 진행하니까 시간이 조금 부족했지만 주어진 시간 안에 역할을 분배하고 맡은 역할을 해낸 것 같아 뿌듯합니다. 데이터베이스 설계시 중복된 내용을 줄이기 위해서 하나의 테이블에 id 값만 가지는 테이블을 join 해서 사용했는데, 구현하다보니까 되게 복잡해졌고 처음에 테이블 설계할 때 제대로 설계하는 것이 중요하다는 것을 몸소 느꼈습니다. 다들 고생하셨습니다 !|
